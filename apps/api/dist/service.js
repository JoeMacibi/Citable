...
async function getOrCreateWebsite(domain, organizationId) {
    try {
...
export async function startAuditJob(payload) {
    ...
    try {
        ...
        console.warn('[Audit Service] Notice: DB offline or skipping persistence:', err.message);
    }
    // Attempt BullMQ enqueue, with graceful fallback to background direct execution
    try {
        await crawlQueue.add('crawl-site', { auditId: id, domain: normalizedDomain, intent: payload.intent }, { removeOnComplete: 100, removeOnFail: 200 });
    }
    catch (queueErr) {
        console.warn('[Queue Notice] BullMQ/Redis offline; executing crawler in standalone mode:', queueErr.message);
        setImmediate(async () => {
...
export async function approveAuditFinding(auditId, findingId) {
