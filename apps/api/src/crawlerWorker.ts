...
function extractMetadata(html: string, pageUrl: string) {
  ...
  $('script[type="application/ld+json"]').each((_, el) => {
    ...
    if (text) jsonLdSnippets.push(text);
  });
...
export async function crawlSite(initialDomain: string, onProgress?: (status: 'CRAWLING' | 'ANALYZING', detail?: any) => void) {
  ...
  const pageData: Array<ReturnType<typeof extractMetadata>> = [];
  ...
  while (queue.length > 0 && pageData.length < MAX_PAGES) {
    ...
    try {
      ...
      console.warn(`[Crawler] Notice fetching ${current}:`, (err as Error).message);
    }
  }
  ...
  const visibilityScore = Math.min(
    100,
    Math.max(35, Math.round((technicalScore * 0.65) + schemaBonus + (pageData.length > 1 ? 15 : 8)))
  );
...
export async function executeCrawlJob(payload: CrawlPayload) {
  const { auditId, domain, intent } = payload;

  const updateStatus = async (status: 'CRAWLING' | 'ANALYZING' | 'COMPLETED' | 'FAILED', extra: any = {}) => {
  ...
  try {
    ...
    const generatedFindings: FindingItem[] = [];
...
export const crawlerWorker = new Worker<CrawlPayload>(
  'citable-crawl',
  async (job: Job<CrawlPayload>) => {
