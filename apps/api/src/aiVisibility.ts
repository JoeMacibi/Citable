...
function findIntent(prompt: string): string {
  const normalized = prompt.toLowerCase();
  if (normalized.includes('pricing') || normalized.includes('cost') || normalized.includes('price')) return 'commercial-intent';
  if (normalized.includes('best') || normalized.includes('compare') || normalized.includes('vs')) return 'research-intent';
  if (normalized.includes('how to') || normalized.includes('guide')) return 'learning-intent';
  return 'informational';
}

export function evaluateAiVisibility(prompt: string, metadata: { model?: string; organizationId?: number } = {}): AiVisibilityResult {
